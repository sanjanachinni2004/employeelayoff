...
def load_data(uploaded_file=None):
    if uploaded_file is None:
        data = pd.read_excel("employee_layoff_dataset_large.xlsx")
    else:
...
def main():
    ...
    company_names_list = data['Company Name'].unique().tolist()[:15]
    job_roles_list = data['Job Role'].unique().tolist()
    regions_list = data['Region'].unique().tolist()
    departments_list = data['Department'].unique().tolist()
    employment_status_list = data['Employment Status'].unique().tolist()
    ...
    if user_type == "Employee":
        ...
        company_name = st.selectbox("Company Name", ['Select'] + sorted(company_names_list))
        region = st.selectbox("Region", ['Select'] + sorted(regions_list))
        job_role = st.selectbox("Job Role", ['Select'] + sorted(job_roles_list))
        department = st.selectbox("Department", ['Select'] + sorted(departments_list))
        employment_type = st.selectbox("Employment Type", ['Select', 'Full-time', 'Part-time', 'Contract'])
        voluntary_exit = st.selectbox("Voluntary Exit", ['Select', 'Yes', 'No'])
        employment_status = st.selectbox("Employment Status", ['Select'] + sorted(employment_status_list))
        ...
        number_of_projects = st.number_input("Number of Projects Completed", min_value=0)
        ...
        if st.button("Predict Layoff Probability"):
            if 'Select' in [gender, marital_status, company_name, region, job_role, department, employment_type, voluntary_exit, employment_status]:
                ...
                input_data = pd.DataFrame([[gender, marital_status, company_name, region, job_role,
                                            department, employment_type, voluntary_exit, employment_status,
                                            years_at_company, years_since_last_promotion, number_of_projects, ctc_inr]],
                                          columns=['Gender', 'Marital Status', 'Company Name', 'Region', 'Job Role',
                                                   'Department', 'Employment Type', 'Voluntary Exit', 'Employment Status',
                                                   'Years at Company', 'Years Since Last Promotion', 'Number of Projects Completed',
                                                   'Current Salary (CTC in INR)'])
                ...
                if layoff_prob > 0.75:
                    st.error("High risk of layoff.")
                elif layoff_prob > 0.65:
                    st.warning("Moderate risk. Consider improving skills or performance.")
                else:
                    st.success("Low risk of layoff. Keep up the good work!")

    elif user_type == "HR":
        st.header("HR Dashboard")
        uploaded_file = st.file_uploader("Upload Employee Data (.xlsx)", type=['xlsx'])
        ...
        if uploaded_file:
            ...
            if 'Layoff Status' not in hr_data.columns:
                st.error("Dataset must contain 'Layoff Status' column.")
                return
            ...
            for col in hr_data.select_dtypes(include=['object']).columns:
                if col in label_encoders:
                    hr_data[col] = label_encoders[col].transform(hr_data[col].astype(str))
            ...
            st.subheader("Layoff Predictions")
